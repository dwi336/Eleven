...
public class LetterTileDrawable extends Drawable {
    ...
    private static synchronized void initializeStaticVariables(final Context context) {
        if (sColors == null) {
            ...
            final Drawable artistDrawable = AppCompatResources.getDrawable(context,
                    R.drawable.ic_artist);
            final Drawable albumDrawable = AppCompatResources.getDrawable(context,
                    R.drawable.ic_album);
            final Drawable playlistDrawable = AppCompatResources.getDrawable(context,
    ...
    private static void drawBitmap(final Bitmap bitmap, final int width, final int height,
                                   final Canvas canvas, final Rect destRect,
                                   final float offset, final Paint paint) {
        // Crop the destination bounds into a square, scaled and offset as appropriate
        final int halfLength = (int) ((float) Math.min(destRect.width(), destRect.height()) / 2);
