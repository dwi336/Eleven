 ...
 *
 * Note that this is not totally up to date. But probably good enough.
 * Newer data is here: http://unicode.org/repos/cldr/trunk/common/supplemental/supplementalData.xml
 * There is functionality around to read the supplementalData.xml file in Java.
 *
 * Fabian does not understand what's with the Runic script:
 * scriptsByLocale.put("de", getScriptsMap("", "Latn", "BR", "Runr", "KZ", "Runr", "US", "Runr"));
 * Same in the xml file, even more countries:
 ...
 * Runic is a dead script see http://scriptsource.org/cms/scripts/page.php?item_id=script_detail&key=Runr
 *
 * Fabian's changes:
 *  - Clear defined API returning Optional.
 *    Because the ISO 639-1 standard is not frozen, it can and does change, and this is not a code
 *    validation class.
 *  - Fix for 3 language codes that have changed, including Hebrew
 *  - Fix for requesting language/country combo where the data has no override for the country, so that
 *    the default of the language is used
 *  - Optimization in data structure (compact) by defining required map size, and compacting with immutable.
 *  - Requires Guava for immutable maps and Optional, or else Java 8 without immutable maps.
 *  - Added tests.
 *
 *
 * @author Phil Brown Dec 20, 2013
 * @author Fabian Kessler
 * @license apache2 license
 */
public class LocaleUtility {

    private static Map<String, Map<String, String>> scriptsByLocale = init();

    private static Map<String, String> getScriptsMap(String... keyValuePairs) {
        Map<String, String> languages = new HashMap<>(keyValuePairs.length/2);
...
//        return languages;
    ...
    private static Map<String, Map<String, String>> init() {
        Map<String, Map<String, String>> scriptsByLocale = new HashMap<>();
        ...
        //with Guava:
        return ImmutableMap.copyOf(scriptsByLocale);

        //without Guava:
        //return scriptsByLocale;
    }

    /**
     * Gets the script for the given locale.
     *
     * Note that some codes are weird, see class header.
     * For example, if a US citizen uses German Locale, and calls this method with Locale.getDefault(),
     * the result would be "Runr".
     *
     * @param locale Usually just the language, possibly with the country.
     *               Examples: new Locale("de"), new Locale("de", "DE")
     * @return Absent if either the method doesn't know, or the input is invalid.
     *         Because this is not a validation method for language and country codes, and new codes may
     *         appear at any time, it's best for this method to be agnostic to it and just say it does not know.
     */
    public static String getScript(Locale locale) {
        ...
        //fixing Java Locale code crap, see http://stackoverflow.com/questions/13974169/jdk-locale-class-handling-of-iso-language-codes-for-hebrew-he-yiddish-yi-an
        ...
        String script = null;
        if (scripts!=null) {
            script = scripts.get(country);
            if (script==null && !country.isEmpty()) {
                //fall back to default, without country:
                script = scripts.get("");
            }
            if (script!=null && script.isEmpty()) script = null;
        }
        return script;
    }
    ...
    /**
     * Overloaded method using just the language.
     */
